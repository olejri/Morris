...
public class GameHandler {
	...
	// chooseFeeDialog() method - opens a dialog where u state the fee for the new game
	private void chooseFeeDialog(){
		skMorris.getUIManager().showChooseFeeScreen(menuContext, new SKOnFeeChosenListener() 
		{		
			@Override
	...
	{	
		...
		skMorris.getGameManager().getTurnBasedTools().getFeeOptions(new SKOnGetFeeOptionsListener()
		{	
			...
			private void showChooseFeeDialog(final String title,final String text)
			{
				AlertDialog.Builder alert = new AlertDialog.Builder(menuContext);           
				...
				{     
					...
					{      
						...
						//invokes the CreateNewGame() method of the TurnBasedGames object.
						//this method creates a new game that is represented in the lobby and opened for other users to join
